...
export default function OrderDetailsPage() {
  ...
  // Fallback seller: if not in sellers list, build from order/product data
  // This handles products from sellers not in the mock DB (e.g. demo_seller)
  const seller = sellers.find((s: any) => String(s.id) === String(sellerId)) || (sellerId ? {
    id: String(sellerId),
    name: order?.seller_name || order?.seller_name_snapshot || (product as any)?.sellerName || 'Vendeur',
    avatar: (product as any)?.sellerAvatar || '',
    storeBg: (product as any)?.sellerStoreBg || '',
    isVerified: false,
    isOnline: false,
    rating: 0,
    totalSales: 0,
  ...
  // Buyer name (if available) + quantity for the invoice
  const buyerName = order?.buyer_name || order?.buyerName || '';
  const quantity = order?.quantity || order?.qty || 1;
  const shippingLabel = lb('Not included', 'Non incluse', 'غير مشمول');

  const handleExportInvoice = async () => {
    if (exporting) return;
    setExporting(true);
    try {
      // Request media library permission (Android 13+ / iOS)
      if (Platform.OS !== 'web') {
        const perm = await MediaLibrary.requestPermissionsAsync();
        if (!perm.granted) {
          Alert.alert(
            lb('Permission needed', 'Permission requise', 'الإذن مطلوب'),
            lb('Please allow photo access to save the invoice.', 'Veuillez autoriser l\'accès aux photos pour enregistrer la facture.', 'يرجى السماح بالوصول إلى الصور لحفظ الفاتورة.'),
          );
          return;
        }
      }
      ...
      // Saved silently — no disruptive popup. The user can find the invoice in their gallery.
    } catch (e) {
      console.log('Export invoice error:', e);
      Alert.alert(
        lb('Export failed', 'Échec de l\'export', 'فشل التصدير'),
        lb('Could not save the invoice. Please try again.', 'Impossible d\'enregistrer la facture. Veuillez réessayer.', 'تعذر حفظ الفاتورة. يرجى المحاولة مرة أخرى.'),
      );
    } finally {
      setExporting(false);
    }
    ...
    <SafeAreaView edges={['top']} style={[sStyles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: scale(40), paddingHorizontal: scale(16) }}>
        ...
        <View style={[sStyles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <View style={[sStyles.productRow, isAr && { flexDirection: 'row-reverse' }]}>
            {productImage ? (
        ...
        <View style={[sStyles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <View style={[sStyles.sellerRow, isAr && { flexDirection: 'row-reverse' }]}>
            {seller?.avatar ? (
        ...
        {/* Action buttons */}
        <View style={[sStyles.actionsRow, isAr && { flexDirection: 'row-reverse' }]}>
          {/* View Product */}
          <Pressable
        ...
        <Pressable
          ...
          disabled={exporting}
          style={({ pressed }) => [sStyles.exportBtn, { backgroundColor: colors.success, opacity: exporting ? 0.6 : pressed ? 0.88 : 1 }]}
        >
          <MaterialIcons name="receipt-long" size={scale(18)} color="#FFF" />
          <Text style={[sStyles.exportBtnText, { color: '#FFF', fontFamily: 'Cairo-SemiBold' }]}>
            {exporting
              ? lb('Exporting…', 'Export en cours…', 'جارٍ التصدير…')
              : lb('Export Invoice', 'Exporter la facture', 'تصدير الفاتورة')}
          </Text>
        </Pressable>
      </ScrollView>

      {/* Hidden invoice document — captured as an image via view-shot */}
      <View style={sStyles.hiddenInvoice} pointerEvents="none">
        <View ref={invoiceRef} collapsable={false}>
          <InvoiceDocument
            transactionNumber={transactionNumber}
            dateLabel={createdAt ? new Date(createdAt).toLocaleDateString(isFr ? 'fr-FR' : isAr ? 'ar' : 'en-US', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}
            ...
            sellerName={seller?.name || order?.seller_name || order?.seller_name_snapshot || lb('Seller', 'Vendeur', 'البائع')}
            sellerVerified={Boolean(seller?.isVerified)}
            sellerAvatar={seller?.avatar || ''}
            storeBg={seller?.storeBg}
            buyerName={buyerName}
...
const sStyles = StyleSheet.create({
  ...
  exportBtn: {
    ...
    paddingVertical: scale(14), borderRadius: scale(12), marginTop: scale(12),
  },
  exportBtnText: { fontSize: scale(14), fontWeight: '700' },
  hiddenInvoice: {
    position: 'absolute',
    left: -10000,
    top: 0,
