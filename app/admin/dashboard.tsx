...
import {
  View, Text, StyleSheet, ScrollView, Pressable, TextInput, Modal, Alert, Switch, ActivityIndicator, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function AdminDashboardScreen() {
  ...
  const {
    ...
    toggleBanSeller, banUserWithDuration, permanentBanUser, addPaymentMethod, updatePaymentMethodLogo, updatePaymentMethod, togglePaymentMethodActive, removePaymentMethod,
  ...
  const [editPaymentColor, setEditPaymentColor] = useState('#E4002B');
  ...
  const openEditPayment = useCallback((method: any) => {
    setEditPaymentId(method.id);
    setEditPaymentName(method.name || '');
    setEditPaymentColor(method.color || '#E4002B');
    setEditPaymentInstructions(method.instructions || '');
    setShowEditPaymentModal(true);
  }, []);

  const handleSaveEditPayment = useCallback(() => {
    if (!editPaymentName.trim()) { Alert.alert(lb('Name required', 'Nom requis', 'الاسم مطلوب')); return; }
    notifySuccess();
    updatePaymentMethod(editPaymentId, {
      name: editPaymentName.trim(),
      color: editPaymentColor,
      instructions: editPaymentInstructions.trim(),
    });
    setShowEditPaymentModal(false);
  }, [editPaymentId, editPaymentName, editPaymentColor, editPaymentInstructions, updatePaymentMethod, lb]);
  ...
  const renderSellersTab = () => (
    <View style={styles.tabContent}>
      ...
      <Pressable onPress={() => refreshUsers()} style={[styles.addBtn, { backgroundColor: colors.primary, marginBottom: scale(8) }]}>
        <MaterialIcons name="refresh" size={scale(22)} color="#FFF" />
        <Text style={styles.addBtnText}>{lb('Refresh Users', 'Rafraîchir', 'تحديث المستخدمين')}</Text>
      </Pressable>
      <View style={[styles.searchBar, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        <MaterialIcons name="search" size={scale(22)} color={colors.textTertiary} />
        ...
        {sellerSearch ? <Pressable onPress={() => setSellerSearch('')} hitSlop={scale(8)}><MaterialIcons name="close" size={scale(20)} color={colors.textTertiary} /></Pressable> : null}
      </View>
      {filteredSellers.length === 0 ? (
        <View style={styles.emptyState}><MaterialIcons name="search-off" size={scale(48)} color={colors.textTertiary} /><Text style={[styles.emptyText, { color: colors.textSecondary }]}>{lb('No sellers found', 'Aucun vendeur trouvé', 'لم يتم العثور على بائع')}</Text></View>
          ...
          <View key={seller.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: seller.isBanned ? colors.error : colors.border }]}>
            <View style={styles.cardHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {seller.email ? <Text style={[styles.cardMeta, { color: colors.textTertiary, fontSize: scale(12) }]}>{seller.email}</Text> : null}
                ...
                {seller.role ? (
                  <View style={[styles.bannedBadge, { backgroundColor: seller.role === 'super_admin' ? colors.error + '15' : seller.role === 'seller' ? colors.primary + '15' : colors.secondary + '15', marginTop: scale(4) }]}>
                    <Text style={[styles.bannedText, { color: seller.role === 'super_admin' ? colors.error : seller.role === 'seller' ? colors.primary : colors.secondary, fontSize: scale(9) }]}>{seller.role.toUpperCase()}</Text>
                  </View>
                ) : null}
            ...
            <View style={styles.cardActions}>
              <Pressable onPress={() => handleVerifySeller(seller.id)} style={[styles.actionBtn, { backgroundColor: seller.isVerified ? colors.verified + '15' : colors.surface, borderColor: seller.isVerified ? colors.verified : colors.border }]}>
                <MaterialIcons name={seller.isVerified ? 'verified' : 'verified-user'} size={scale(16)} color={seller.isVerified ? colors.verified : colors.textSecondary} />
              ...
              <Pressable onPress={() => openBanModal(seller.id)} style={[styles.actionBtn, { backgroundColor: seller.isBanned ? colors.successLight : colors.errorLight, borderColor: seller.isBanned ? colors.success : colors.error }]}>
                <MaterialIcons name={seller.isBanned ? 'lock-open' : 'block'} size={scale(16)} color={seller.isBanned ? colors.success : colors.error} />
              ...
              {!seller.isBanned ? (
                <Pressable onPress={() => {
                  ...
                  <MaterialIcons name="gavel" size={scale(16)} color="#FFF" />
              ...
              <Pressable onPress={() => {
                ...
                <MaterialIcons name="star" size={scale(16)} color="#F59E0B" />
    ...
    return (
      <View style={styles.tabContent}>
        {allDisputed.length === 0 ? (
          <View style={styles.emptyState}><MaterialIcons name="check-circle" size={scale(48)} color={colors.success} /><Text style={[styles.emptyText, { color: colors.textSecondary }]}>{lb('No active disputes', 'Aucun litige en cours', 'لا توجد نزاعات نشطة')}</Text></View>
          ...
          return (
            <View key={order.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: order.status === 'disputed' ? colors.error : colors.border }]}>
              ...
              <View style={styles.disputeParties}>
                <View style={styles.partyRow}><MaterialIcons name="person" size={scale(14)} color={colors.textTertiary} /><Text style={[styles.partyLabel, { color: colors.textTertiary }]}>{lb('Buyer:', 'Acheteur:', 'المشتري:')}</Text><Text style={[styles.partyValue, { color: colors.textPrimary }]}>{order.buyerPhone}</Text></View>
                <View style={styles.partyRow}><MaterialIcons name="store" size={scale(14)} color={colors.textTertiary} /><Text style={[styles.partyLabel, { color: colors.textTertiary }]}>{lb('Seller:', 'Vendeur:', 'البائع:')}</Text><Text style={[styles.partyValue, { color: colors.textPrimary }]}>{seller ? seller.name : 'N/A'}</Text></View>
              </View>
              <View style={styles.cardActions}>
                <Pressable onPress={() => { impactLight(); updateOrderStatus(order.id, 'confirmed'); }} style={[styles.actionBtn, { backgroundColor: colors.successLight, borderColor: colors.success }]}>
                  <MaterialIcons name="check" size={scale(16)} color={colors.success} /><Text style={[styles.actionText, { color: colors.success }]}>{lb('Confirm', 'Confirmer', 'تأكيد')}</Text>
                </Pressable>
                {order.status !== 'disputed' ? (
                  <Pressable onPress={() => { impactMedium(); updateOrderStatus(order.id, 'disputed'); }} style={[styles.actionBtn, { backgroundColor: colors.errorLight, borderColor: colors.error }]}>
                    <MaterialIcons name="gavel" size={scale(16)} color={colors.error} /><Text style={[styles.actionText, { color: colors.error }]}>{lb('Dispute', 'Disputer', 'نزاع')}</Text>
                  </Pressable>
                ) : null}
              </View>
            </View>
  ...
  const renderPaymentsTab = () => (
    <View style={styles.tabContent}>
      <Pressable onPress={() => { setNewPaymentLogo(''); setShowPaymentModal(true); }} style={[styles.addBtn, { backgroundColor: colors.primary }]}>
        <MaterialIcons name="add" size={scale(22)} color="#FFF" />
        <Text style={styles.addBtnText}>{lb('Add Payment Method', 'Ajouter une methode', 'إضافة طريقة دفع')}</Text>
      </Pressable>
      {paymentMethodsList.map(method => (
        <View key={method.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: (method.isActive ?? true) ? colors.border : colors.error + '40' }]}>
          <View style={styles.paymentRow}>
            {method.logo ? (
              ...
              <View style={[styles.paymentIcon, { backgroundColor: method.color + '20' }]}>
                <MaterialIcons name="account-balance-wallet" size={scale(24)} color={method.color} />
              </View>
            )}
            <View style={{ flex: 1 }}>
              <View style={styles.nameRow}>
                <Text style={[styles.cardName, { color: colors.textPrimary }]}>{method.name}</Text>
                <View style={[styles.statusDot, { backgroundColor: (method.isActive ?? true) ? '#10B981' : '#EF4444' }]} />
              </View>
              {method.instructions ? (
            ...
            <View style={{ flexDirection: 'row', gap: scale(6) }}>
              <Pressable onPress={() => openEditPayment(method)} style={[styles.smallIconBtn, { backgroundColor: colors.warning + '15' }]} hitSlop={scale(8)}>
                <MaterialIcons name="edit" size={scale(18)} color={colors.warning} />
              </Pressable>
              ...
              }} style={[styles.smallIconBtn, { backgroundColor: colors.verified + '15' }]} hitSlop={scale(8)}>
                <MaterialIcons name="image" size={scale(18)} color={colors.verified} />
              </Pressable>
              <Pressable onPress={() => handleRemovePayment(method.id, method.name)} style={[styles.smallIconBtn, { backgroundColor: colors.errorLight }]} hitSlop={scale(8)}>
                <MaterialIcons name="delete-outline" size={scale(18)} color={colors.error} />
              </Pressable>
            </View>
          </View>
          <View style={styles.cardActions}>
            <Pressable onPress={() => { selection(); togglePaymentMethodActive(method.id); }} style={[styles.actionBtn, { backgroundColor: (method.isActive ?? true) ? colors.warningLight : colors.successLight, borderColor: (method.isActive ?? true) ? colors.warning : colors.success }]}>
              <MaterialIcons name={(method.isActive ?? true) ? 'pause-circle' : 'play-circle'} size={scale(16)} color={(method.isActive ?? true) ? colors.warning : colors.success} />
              <Text style={[styles.actionText, { color: (method.isActive ?? true) ? colors.warning : colors.success }]}>{(method.isActive ?? true) ? lb('Disable', 'Desactiver', 'تعطيل') : lb('Enable', 'Activer', 'تفعيل')}</Text>
            </Pressable>
          </View>
        </View>
      ))}
  ...
  const renderShippingTab = () => (
    <View style={styles.tabContent}>
      ...
      {shippingCompanies.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialIcons name="local-shipping" size={scale(48)} color={colors.textTertiary} />
        ...
        <View key={ship.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: ship.isActive ? colors.border : colors.error + '40' }]}>
          <View style={styles.cardHeader}>
            <View style={[styles.statIcon, { backgroundColor: '#8B5CF620' }]}>
              <MaterialIcons name="local-shipping" size={scale(20)} color="#8B5CF6" />
            </View>
            <View style={{ flex: 1 }}>
              <View style={styles.nameRow}>
          ...
          <View style={styles.cardActions}>
            <Pressable onPress={() => toggleShippingCompanyActive(ship.id)} style={[styles.actionBtn, { backgroundColor: ship.isActive ? colors.warningLight : colors.successLight, borderColor: ship.isActive ? colors.warning : colors.success }]}>
              <MaterialIcons name={ship.isActive ? 'pause-circle' : 'play-circle'} size={scale(16)} color={ship.isActive ? colors.warning : colors.success} />
            ...
            <Pressable onPress={() => {
              ...
              <MaterialIcons name="delete" size={scale(16)} color={colors.error} />
  ...
  const renderUsersTab = () => (
    <View style={styles.tabContent}>
      <View style={[styles.onlineOfflineRow, { marginBottom: scale(8) }]}>
      ...
      <View style={[styles.searchBar, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        ...
        {userSearch ? <Pressable onPress={() => setUserSearch('')} hitSlop={scale(8)}><MaterialIcons name="close" size={scale(20)} color={colors.textTertiary} /></Pressable> : null}
      </View>
  ...
  const renderBlacklistTab = () => (
    <View style={styles.tabContent}>
      {blacklist.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialIcons name="verified-user" size={scale(48)} color={colors.success} />
        ...
        <View key={idx} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.error + '40' }]}>
          ...
          <View style={styles.blacklistDetails}>
            <View style={styles.blacklistRow}>
              <MaterialIcons name="email" size={scale(14)} color={colors.textTertiary} />
              <Text style={[styles.blacklistValue, { color: colors.textSecondary }]}>{entry.email}</Text>
            </View>
            <View style={styles.blacklistRow}>
              <MaterialIcons name="phone" size={scale(14)} color={colors.textTertiary} />
  ...
  const renderStaffTab = () => (
    <View style={styles.tabContent}>
      <Pressable onPress={() => { setNewStaffPermissions([]); setShowStaffModal(true); }} style={[styles.addBtn, { backgroundColor: '#8B5CF6' }]}>
        <MaterialIcons name="person-add" size={scale(22)} color="#FFF" />
        <Text style={styles.addBtnText}>{lb('Add Staff Member', 'Ajouter un employe', 'إضافة موظف')}</Text>
      </Pressable>
      {staffMembers.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialIcons name="group-add" size={scale(48)} color={colors.textTertiary} />
        ...
        <View key={member.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: member.isActive ? colors.border : colors.error + '40' }]}>
          <View style={styles.cardHeader}>
            <View style={[styles.staffAvatar, { backgroundColor: member.isActive ? '#8B5CF620' : colors.errorLight }]}>
              <MaterialIcons name="admin-panel-settings" size={scale(24)} color={member.isActive ? '#8B5CF6' : colors.error} />
            </View>
            <View style={{ flex: 1 }}>
              <View style={styles.nameRow}>
          ...
          <View style={styles.cardActions}>
            <Pressable onPress={() => toggleStaffActive(member.id)} style={[styles.actionBtn, { backgroundColor: member.isActive ? colors.warningLight : colors.successLight, borderColor: member.isActive ? colors.warning : colors.success }]}>
              <MaterialIcons name={member.isActive ? 'pause-circle' : 'play-circle'} size={scale(16)} color={member.isActive ? colors.warning : colors.success} />
  ...
  const renderBrandingTab = () => (
    <View style={styles.tabContent}>
      ...
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.primary + '30' }]}>
        ...
        <View style={{ alignItems: 'center', gap: scale(12), marginTop: scale(8) }}>
          {storeLogo ? (
            <View style={{ alignItems: 'center', gap: scale(8) }}>
              <Image source={{ uri: storeLogo }} style={{ width: scale(80), height: scale(80), borderRadius: scale(16) }} contentFit="cover" />
            ...
            <View style={{ alignItems: 'center', gap: scale(4) }}>
              <MaterialIcons name="image-not-supported" size={scale(40)} color={colors.textTertiary} />
              <Text style={[styles.cardMeta, { color: colors.textTertiary }]}>{lb('No logo uploaded', 'Aucun logo', '\u0644\u0645 \u064a\u062a\u0645 \u0631\u0641\u0639 \u0634\u0639\u0627\u0631')}</Text>
            </View>
          )}
          <Pressable onPress={handlePickStoreLogo} style={[styles.addBtn, { backgroundColor: colors.primary, height: scale(44) }]}>
            <MaterialIcons name={storeLogo ? 'edit' : 'add-a-photo'} size={scale(18)} color="#FFF" />
            <Text style={[styles.addBtnText, { fontSize: scale(14) }]}>{logoLoading ? '...' : storeLogo ? lb('Change Logo', 'Changer le logo', '\u062a\u063a\u064a\u064a\u0631 \u0627\u0644\u0634\u0639\u0627\u0631') : lb('Upload Logo', 'Telecharger le logo', '\u0631\u0641\u0639 \u0627\u0644\u0634\u0639\u0627\u0631')}</Text>
          </Pressable>
        </View>
      </View>
      ...
      <Pressable onPress={() => { setNewBannerTitle(''); setNewBannerImage(''); setNewBannerLink(''); setShowAddBannerModal(true); }} style={[styles.addBtn, { backgroundColor: '#F59E0B' }]}>
        <MaterialIcons name="add-photo-alternate" size={scale(22)} color="#FFF" />
        <Text style={styles.addBtnText}>{lb('Add Banner', 'Ajouter une banniere', '\u0625\u0636\u0627\u0641\u0629 \u0628\u0627\u0646\u0631')}</Text>
      </Pressable>

      {adBanners.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialIcons name="image" size={scale(48)} color={colors.textTertiary} />
        ...
        <View key={banner.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: banner.is_active ? colors.border : colors.error + '40' }]}>
          <Image source={{ uri: banner.image_url }} style={{ width: '100%', height: scale(100), borderRadius: scale(8) }} contentFit="cover" />
          ...
          <View style={styles.cardActions}>
            {idx > 0 ? (
              <Pressable onPress={() => handleMoveBanner(idx, 'up')} style={[styles.actionBtn, { backgroundColor: colors.surface, borderColor: colors.border, flex: 0, paddingHorizontal: scale(12) }]}>
                <MaterialIcons name="arrow-upward" size={scale(16)} color={colors.textSecondary} />
              </Pressable>
            ) : null}
            {idx < adBanners.length - 1 ? (
              <Pressable onPress={() => handleMoveBanner(idx, 'down')} style={[styles.actionBtn, { backgroundColor: colors.surface, borderColor: colors.border, flex: 0, paddingHorizontal: scale(12) }]}>
                <MaterialIcons name="arrow-downward" size={scale(16)} color={colors.textSecondary} />
              </Pressable>
            ) : null}
            <Pressable onPress={() => handleToggleBanner(banner.id, banner.is_active)} style={[styles.actionBtn, { backgroundColor: banner.is_active ? colors.warningLight : colors.successLight, borderColor: banner.is_active ? colors.warning : colors.success }]}>
              <MaterialIcons name={banner.is_active ? 'visibility-off' : 'visibility'} size={scale(16)} color={banner.is_active ? colors.warning : colors.success} />
  ...
  const renderVerificationTab = () => (
    <View style={styles.tabContent}>
      ...
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.primary + '30' }]}>
        <View style={styles.cardHeader}>
          <View style={[styles.statIcon, { backgroundColor: colors.primary + '15' }]}>
            <MaterialIcons name="card-membership" size={scale(20)} color={colors.primary} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={[styles.cardName, { color: colors.textPrimary }]}>
        ...
        {verificationPlans.map(plan => (
          <View key={plan.id} style={{ flexDirection: 'row', alignItems: 'center', gap: scale(10), paddingVertical: scale(8), borderTopWidth: 1, borderTopColor: colors.borderLight }}>
            <View style={[styles.statIcon, { backgroundColor: plan.isActive ? colors.success + '15' : colors.error + '15' }]}>
              <MaterialIcons name="verified" size={scale(16)} color={plan.isActive ? colors.success : colors.error} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[styles.cardName, { color: colors.textPrimary, fontSize: scale(14) }]}>{plan.name}</Text>
            ...
            <Pressable onPress={() => toggleVerificationPlanActive(plan.id)} style={[styles.smallIconBtn, { backgroundColor: plan.isActive ? colors.warningLight : colors.successLight }]} hitSlop={scale(8)}>
              <MaterialIcons name={plan.isActive ? 'pause' : 'play-arrow'} size={scale(18)} color={plan.isActive ? colors.warning : colors.success} />
            </Pressable>
            <Pressable onPress={() => {
            ...
            }} style={[styles.smallIconBtn, { backgroundColor: colors.errorLight }]} hitSlop={scale(8)}>
              <MaterialIcons name="delete-outline" size={scale(18)} color={colors.error} />
            </Pressable>
          </View>
        ))}
        <Pressable onPress={() => { setNewPlanName(''); setNewPlanDays(''); setNewPlanPrice(''); setShowPlanModal(true); }} style={[styles.addBtn, { backgroundColor: colors.verified, height: scale(44), marginTop: scale(8) }]}>
          <MaterialIcons name="add" size={scale(20)} color="#FFF" />
          <Text style={[styles.addBtnText, { fontSize: scale(14) }]}>{lb('Add Plan', 'Ajouter un plan', 'إضافة خطة')}</Text>
        </Pressable>
      </View>

      {/* Filter chips */}
      <View style={{ flexDirection: 'row', gap: scale(8), marginBottom: scale(4), marginTop: scale(8) }}>
      ...
      {filteredVerifyRequests.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialIcons name="verified" size={scale(48)} color={colors.textTertiary} />
          ...
          <View key={sub.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: statusColor + '40' }]}>
            <View style={styles.cardHeader}>
              <View style={[styles.statIcon, { backgroundColor: statusColor + '15' }]}>
                <MaterialIcons name={statusIcon as any} size={scale(20)} color={statusColor} />
              </View>
              <View style={{ flex: 1 }}>
            ...
            {sub.screenshotUri ? (
              <View>
                <Text style={[styles.cardMeta, { color: colors.textSecondary, fontWeight: '600', marginBottom: scale(4) }]}>
                ...
                <Pressable onPress={() => { setVerifyImageUrl(sub.screenshotUri); setShowVerifyImageModal(true); }}>
                  <Image source={{ uri: sub.screenshotUri }} style={{ width: '100%', height: scale(140), borderRadius: scale(8) }} contentFit="cover" transition={200} />
                </Pressable>
              </View>
            ) : null}

            {sub.adminNotes ? (
              <View style={{ paddingHorizontal: scale(8), paddingVertical: scale(6), backgroundColor: colors.backgroundSecondary, borderRadius: scale(6) }}>
            ...
            {sub.status === 'pending' ? (
              <View style={styles.cardActions}>
                ...
                <Pressable onPress={() => handleOpenRejectModal(sub.id)}
                  style={[styles.actionBtn, { backgroundColor: colors.errorLight, borderColor: colors.error }]}>
                  <MaterialIcons name="cancel" size={scale(16)} color={colors.error} />
  ...
  const renderLanguagesTab = () => {
    ...
    return (
      <View style={{ padding: scale(16), gap: scale(12) }}>
        <Text style={{ fontSize: scale(14), fontWeight: '700', color: colors.textSecondary, marginBottom: scale(4) }}>
        ...
        {LANG_LIST.map(lang => {
          ...
          return (
            <View key={lang.id} style={{ flexDirection: 'row', alignItems: 'center', padding: scale(14), borderRadius: scale(12), backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, gap: scale(12) }}>
              <Text style={{ fontSize: scale(24) }}>{lang.flag}</Text>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: scale(15), fontWeight: '700', color: colors.textPrimary }}>{lang.name}</Text>
                <Text style={{ fontSize: scale(12), color: colors.textTertiary }}>{lang.desc} / {lang.descFr}</Text>
              </View>
              <Pressable
              ...
              >
                <View style={{ paddingHorizontal: scale(16), paddingVertical: scale(8), borderRadius: scale(8), backgroundColor: isEnabled ? colors.success : colors.border }}>
                  <Text style={{ color: '#FFF', fontSize: scale(13), fontWeight: '700' }}>
        ...
        <Text style={{ fontSize: scale(11), color: colors.textTertiary, marginTop: scale(4) }}>
          {language === 'fr'
    ...
    return (
      <View style={styles.tabContent}>
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.primary + '30' }]}>
          <View style={styles.cardHeader}>
            <View style={[styles.statIcon, { backgroundColor: colors.primary + '15' }]}>
              <MaterialIcons name="public" size={scale(20)} color={colors.primary} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[styles.cardName, { color: colors.textPrimary }]}>
          ...
          return (
            <View key={region.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Pressable onPress={() => toggleRegion(region.id)} style={styles.regionHeader}>
                ...
                <MaterialIcons name={isExpanded ? 'expand-less' : 'expand-more'} size={scale(24)} color={colors.textSecondary} />
              </Pressable>
              {isExpanded ? (
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        <View style={styles.headerLeft}>
          <Pressable onPress={() => router.back()} hitSlop={scale(12)} style={[styles.headerBackBtn, { backgroundColor: colors.backgroundSecondary }]}><MaterialIcons name={isAr ? "arrow-forward" : "arrow-back"} size={scale(20)} color={colors.textPrimary} /></Pressable>
          <View>
        ...
        <Pressable onPress={() => { logout(); router.replace('/(tabs)'); }} style={[styles.adminBadge, { backgroundColor: colors.error + '12' }]} hitSlop={scale(8)}><MaterialIcons name="logout" size={scale(18)} color={colors.error} /></Pressable>
      </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.statsScroll}>
            {stats.map((stat, i) => (
              <View key={i} style={[styles.statCard, { backgroundColor: colors.surface, borderColor: stat.color + '30', borderWidth: 1 }, shadows.card]}>
                <View style={[styles.statIcon, { backgroundColor: stat.color + '15' }]}><MaterialIcons name={stat.icon as any} size={scale(24)} color={stat.color} /></View>
          ...
          <View style={[styles.totalBanner, { backgroundColor: colors.primary + '08', borderColor: colors.primary + '20' }]}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: scale(10) }}>
              <View style={[styles.bannerIcon, { backgroundColor: colors.primary + '15' }]}>
                <MaterialIcons name="groups" size={scale(26)} color={colors.primary} />
              </View>
              <View style={{ flex: 1 }}>
              ...
              <Pressable onPress={() => refreshUsers()} style={[styles.refreshBtn, { backgroundColor: colors.primary }]} hitSlop={scale(8)}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabScroll}>
          {visibleTabs.map(tab => (
            <Pressable key={tab.key} onPress={() => { selection(); setActiveTab(tab.key); }} style={[styles.tabChip, { backgroundColor: activeTab === tab.key ? colors.primary : colors.surface, borderColor: activeTab === tab.key ? colors.primary : colors.border }]}>
              <MaterialIcons name={tab.icon as any} size={scale(18)} color={activeTab === tab.key ? '#FFF' : colors.textSecondary} />
      ...
      <Modal visible={showAddBannerModal} transparent animationType="fade" onRequestClose={() => setShowAddBannerModal(false)}>
        <View style={[styles.modalOverlay, { backgroundColor: colors.overlay }]}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            }} style={[styles.logoPickerBtn, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border, height: scale(140) }]}>
              {newBannerImage ? (
                <Image source={{ uri: newBannerImage }} style={styles.logoPreview} contentFit="cover" />
              ) : (
                <View style={styles.logoPlaceholder}>
                  <MaterialIcons name="add-photo-alternate" size={scale(36)} color={colors.textTertiary} />
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            {VERIFY_DURATIONS.map(d => (
              <Pressable key={d.days} onPress={() => setSelectedVerifyDuration(d.days)} style={[styles.durationOption, { backgroundColor: selectedVerifyDuration === d.days ? colors.verified + '15' : colors.backgroundSecondary, borderColor: selectedVerifyDuration === d.days ? colors.verified : colors.border }]}>
                ...
                {selectedVerifyDuration === d.days ? <MaterialIcons name="check-circle" size={scale(20)} color={colors.verified} /> : null}
              </Pressable>
            ))}
            <View style={styles.modalBtns}>
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            {BAN_DURATIONS.map(d => (
              <Pressable key={d.days} onPress={() => setSelectedBanDuration(d.days)} style={[styles.durationOption, { backgroundColor: selectedBanDuration === d.days ? colors.error + '15' : colors.backgroundSecondary, borderColor: selectedBanDuration === d.days ? colors.error : colors.border }]}>
                ...
                {selectedBanDuration === d.days ? <MaterialIcons name="check-circle" size={scale(20)} color={colors.error} /> : null}
              </Pressable>
            ))}
            <View style={styles.modalBtns}>
      ...
      <Modal visible={showPaymentModal} transparent animationType="fade" onRequestClose={() => setShowPaymentModal(false)}>
        <View style={[styles.modalOverlay, { backgroundColor: colors.overlay }]}>
          <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: scale(24) }} showsVerticalScrollIndicator={false}>
            <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
              ...
              <Pressable onPress={pickPaymentLogo} style={[styles.logoPickerBtn, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]}>
                {newPaymentLogo ? (
                  ...
                  <View style={styles.logoPlaceholder}>
                    <MaterialIcons name="add-photo-alternate" size={scale(28)} color={colors.textTertiary} />
              ...
              <View style={styles.colorRow}>
                {PAYMENT_COLORS.map(c => (
                  <Pressable key={c} onPress={() => setNewPaymentColor(c)} style={[styles.colorDot, { backgroundColor: c, borderColor: newPaymentColor === c ? colors.textPrimary : 'transparent' }]}>
                    {newPaymentColor === c ? <MaterialIcons name="check" size={scale(16)} color="#FFF" /> : null}
                  </Pressable>
                ))}
              </View>
              <View style={styles.modalBtns}>
      ...
      <Modal visible={showEditPaymentModal} transparent animationType="fade" onRequestClose={() => setShowEditPaymentModal(false)}>
        <View style={[styles.modalOverlay, { backgroundColor: colors.overlay }]}>
          <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: scale(24) }} showsVerticalScrollIndicator={false}>
            <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
              <Text style={[styles.modalTitle, { color: colors.textPrimary }]}>{lb('Edit Payment Method', 'Modifier la methode', 'تعديل طريقة الدفع')}</Text>

              <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{lb('NAME *', 'NOM *', 'الاسم *')}</Text>
              <TextInput style={[styles.modalInput, { backgroundColor: colors.backgroundSecondary, color: colors.textPrimary, borderColor: colors.border }]} placeholder="e.g. Airtel Money" placeholderTextColor={colors.textTertiary} value={editPaymentName} onChangeText={setEditPaymentName} />

              <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{lb('TRANSFER INSTRUCTIONS', 'INSTRUCTIONS', 'تعليمات التحويل')}</Text>
              <TextInput style={[styles.modalInput, styles.textAreaInput, { backgroundColor: colors.backgroundSecondary, color: colors.textPrimary, borderColor: colors.border }]}
                placeholder={lb('e.g. Dial *222# > Send Money...', 'Ex: Composez *222#...', 'مثال: اطلب *222#...')}
                placeholderTextColor={colors.textTertiary} value={editPaymentInstructions} onChangeText={setEditPaymentInstructions} multiline numberOfLines={3} textAlignVertical="top" />

              <Text style={[styles.formLabel, { color: colors.textSecondary }]}>{lb('COLOR', 'COULEUR', 'اللون')}</Text>
              <View style={styles.colorRow}>
                {PAYMENT_COLORS.map(c => (
                  <Pressable key={c} onPress={() => setEditPaymentColor(c)} style={[styles.colorDot, { backgroundColor: c, borderColor: editPaymentColor === c ? colors.textPrimary : 'transparent' }]}>
                    {editPaymentColor === c ? <MaterialIcons name="check" size={scale(16)} color="#FFF" /> : null}
                  </Pressable>
                ))}
              </View>
              <View style={styles.modalBtns}>
                <Pressable onPress={() => setShowEditPaymentModal(false)} style={[styles.modalBtnCancel, { borderColor: colors.border }]}><Text style={[styles.modalBtnCancelText, { color: colors.textSecondary }]}>{lb('Cancel', 'Annuler', 'إلغاء')}</Text></Pressable>
                <Pressable onPress={handleSaveEditPayment} style={[styles.modalBtnConfirm, { backgroundColor: colors.primary }]}><Text style={styles.modalBtnConfirmText}>{lb('Save', 'Enregistrer', 'حفظ')}</Text></Pressable>
              </View>
            </View>
          </ScrollView>
        </View>
              ...
              {ALL_PERMISSIONS.map(perm => {
                ...
                return (
                  <Pressable key={perm.key} onPress={() => togglePermission(perm.key)}
                    ...
                    <MaterialIcons name={perm.icon as any} size={scale(18)} color={isSelected ? '#8B5CF6' : colors.textTertiary} />
                    ...
                    <MaterialIcons name={isSelected ? 'check-box' : 'check-box-outline-blank'} size={scale(22)} color={isSelected ? '#8B5CF6' : colors.textTertiary} />
                  </Pressable>
                );
              })}

              <View style={[styles.modalBtns, { marginTop: scale(12) }]}>
      ...
      <Modal visible={showVerifyImageModal} transparent animationType="fade" onRequestClose={() => setShowVerifyImageModal(false)}>
        <Pressable style={[styles.modalOverlay, { backgroundColor: 'rgba(0,0,0,0.85)' }]} onPress={() => setShowVerifyImageModal(false)}>
          <View style={{ width: '90%', aspectRatio: 3/4, borderRadius: scale(12), overflow: 'hidden' }}>
            {verifyImageUrl ? (
              <Image source={{ uri: verifyImageUrl }} style={{ width: '100%', height: '100%' }} contentFit="contain" />
            ) : (
              <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                <MaterialIcons name="image-not-supported" size={scale(48)} color="#999" />
              </View>
            )}
          </View>
          <Pressable onPress={() => setShowVerifyImageModal(false)} style={{ marginTop: scale(16), paddingHorizontal: scale(24), paddingVertical: scale(12), backgroundColor: '#FFF', borderRadius: scale(8) }}>
            <Text style={{ fontSize: scale(15), fontWeight: '600' }}>{lb('Close', 'Fermer', 'إغلاق')}</Text>
          </Pressable>
        </Pressable>
      </Modal>
      ...
      <Modal visible={showVanityModal} transparent animationType="fade" onRequestClose={() => setShowVanityModal(false)}>
        <View style={[styles.modalOverlay, { backgroundColor: colors.overlay }]}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            ...
            <TextInput style={[styles.modalInput, { backgroundColor: colors.backgroundSecondary, color: colors.textPrimary, borderColor: colors.border, fontSize: scale(22), fontWeight: '800', textAlign: 'center', letterSpacing: 4 }]} placeholder={lb('e.g. 11, 222, 5555', 'Ex: 11, 222, 5555', 'مثال: 11, 222, 5555')} placeholderTextColor={colors.textTertiary} value={vanityNewId} onChangeText={setVanityNewId} keyboardType="numeric" maxLength={10} />
            <View style={[styles.modalBtns, { marginTop: scale(16) }]}>
...
const styles = StyleSheet.create({
  safeArea: { flex: 1 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: scale(16), paddingVertical: scale(12), borderBottomWidth: 1 },
  ...
  headerSub: { fontSize: scale(11), fontWeight: '700', letterSpacing: 0.5 },
  adminBadge: { width: scale(38), height: scale(38), borderRadius: scale(12), alignItems: 'center', justifyContent: 'center' },
  statsScroll: { paddingHorizontal: scale(16), paddingVertical: scale(14), gap: scale(10) },
  statCard: { width: scale(120), paddingVertical: scale(14), paddingHorizontal: scale(12), borderRadius: scale(16), borderWidth: 1, alignItems: 'center', gap: scale(4) },
  statIcon: { width: scale(36), height: scale(36), borderRadius: scale(12), alignItems: 'center', justifyContent: 'center' },
  statValue: { fontSize: scale(24), fontWeight: '800', marginTop: scale(2) },
  statLabel: { fontSize: scale(9), fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.6 },
  tabScroll: { paddingHorizontal: scale(16), gap: scale(6), paddingBottom: scale(14) },
  tabChip: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: scale(14), paddingVertical: scale(10), borderRadius: scale(12), borderWidth: 1.5, gap: scale(6) },
  tabChipText: { fontSize: scale(13), fontWeight: '700' },
  tabContent: { paddingHorizontal: scale(16), gap: scale(12) },
  searchBar: { flexDirection: 'row', alignItems: 'center', height: scale(48), borderRadius: borderRadius.md, paddingHorizontal: scale(14), borderWidth: 1, gap: scale(10) },
  searchInput: { flex: 1, fontSize: scale(15), height: '100%' },
  card: { borderRadius: scale(16), borderWidth: 1, padding: scale(16), gap: scale(12) },
  cardHeader: { flexDirection: 'row', gap: scale(12), alignItems: 'flex-start' },
  avatarWrapper: { position: 'relative' },
  avatar: { width: scale(48), height: scale(48), borderRadius: scale(24) },
  onlineDot: { position: 'absolute', bottom: 0, right: 0, width: scale(14), height: scale(14), borderRadius: scale(7), borderWidth: 2 },
  nameRow: { flexDirection: 'row', alignItems: 'center', gap: scale(6), flexWrap: 'wrap' },
  cardName: { fontSize: scale(16), fontWeight: '700' },
  sellerId: { fontSize: scale(14), fontWeight: '700', marginTop: scale(2) },
  cardMeta: { fontSize: scale(12), marginTop: scale(2) },
  timerText: { fontSize: scale(12), fontWeight: '600', marginTop: scale(4) },
  bannedBadge: { paddingHorizontal: scale(6), paddingVertical: scale(2), borderRadius: scale(4) },
  bannedText: { fontSize: scale(10), fontWeight: '700' },
  cardActions: { flexDirection: 'row', gap: scale(8) },
  actionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: scale(10), borderRadius: scale(10), borderWidth: 1.5, gap: scale(6) },
  actionText: { fontSize: scale(12), fontWeight: '700' },
  disputeHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  statusPill: { paddingHorizontal: scale(10), paddingVertical: scale(4), borderRadius: borderRadius.full },
  statusPillText: { fontSize: scale(11), fontWeight: '700' },
  orderId: { fontSize: scale(12), fontWeight: '500' },
  disputeProduct: { fontSize: scale(15), fontWeight: '600' },
  disputeAmount: { fontSize: scale(20), fontWeight: '800' },
  disputeParties: { gap: scale(4) },
  partyRow: { flexDirection: 'row', alignItems: 'center', gap: scale(6) },
  partyLabel: { fontSize: scale(12), fontWeight: '500' },
  partyValue: { fontSize: scale(13), fontWeight: '600' },
  paymentRow: { flexDirection: 'row', alignItems: 'center', gap: scale(12) },
  paymentIcon: { width: scale(48), height: scale(48), borderRadius: scale(24), alignItems: 'center', justifyContent: 'center' },
  paymentLogoImg: { width: scale(48), height: scale(48), borderRadius: scale(12) },
  smallIconBtn: { width: scale(36), height: scale(36), borderRadius: scale(18), alignItems: 'center', justifyContent: 'center' },
  addBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: scale(50), borderRadius: borderRadius.md, gap: scale(8) },
  addBtnText: { color: '#FFF', fontSize: scale(16), fontWeight: '700' },
  onlineOfflineRow: { flexDirection: 'row', gap: scale(10), marginBottom: scale(12) },
  onlineCard: { flex: 1, flexDirection: 'row', alignItems: 'center', paddingVertical: scale(14), paddingHorizontal: scale(14), borderRadius: borderRadius.md, borderWidth: 1, gap: scale(10) },
  onlineIndicator: { width: scale(10), height: scale(10), borderRadius: scale(5) },
  onlineCardValue: { fontSize: scale(22), fontWeight: '800' },
  onlineCardLabel: { fontSize: scale(12), fontWeight: '500' },
  emptyState: { alignItems: 'center', paddingVertical: scale(48), gap: scale(12) },
  emptyText: { fontSize: scale(16), fontWeight: '500' },
  blacklistHeader: { flexDirection: 'row', alignItems: 'center', gap: scale(8) },
  blacklistDetails: { gap: scale(6), marginLeft: scale(28) },
  blacklistRow: { flexDirection: 'row', alignItems: 'center', gap: scale(6) },
  blacklistValue: { fontSize: scale(13) },
  // Staff
  staffAvatar: { width: scale(48), height: scale(48), borderRadius: scale(24), alignItems: 'center', justifyContent: 'center' },
  statusDot: { width: scale(8), height: scale(8), borderRadius: scale(4) },
  permissionTags: { flexDirection: 'row', flexWrap: 'wrap', gap: scale(4), marginTop: scale(6) },
  permTag: { paddingHorizontal: scale(8), paddingVertical: scale(3), borderRadius: scale(6), borderWidth: 1 },
  permTagText: { fontSize: scale(10), fontWeight: '600' },
  permissionRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: scale(14), paddingVertical: scale(12), borderRadius: borderRadius.sm, borderWidth: 1, marginBottom: scale(6), gap: scale(10) },
  permissionLabel: { flex: 1, fontSize: scale(14), fontWeight: '500' },
  // Modals
  modalOverlay: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  modalContent: { width: '100%', borderRadius: borderRadius.lg, padding: scale(24) },
  modalTitle: { fontSize: scale(20), fontWeight: '700', marginBottom: scale(6) },
  modalDesc: { fontSize: scale(14), lineHeight: scale(21), marginBottom: scale(16) },
  durationOption: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: scale(16), paddingVertical: scale(14), borderRadius: borderRadius.sm, borderWidth: 1, marginBottom: scale(8) },
  durationText: { fontSize: scale(15), fontWeight: '600' },
  modalBtns: { flexDirection: 'row', gap: scale(10), marginTop: scale(8) },
  modalBtnCancel: { flex: 1, height: scale(48), borderRadius: borderRadius.md, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  modalBtnCancelText: { fontSize: scale(15), fontWeight: '600' },
  modalBtnConfirm: { flex: 1, height: scale(48), borderRadius: borderRadius.md, alignItems: 'center', justifyContent: 'center' },
  modalBtnConfirmText: { color: '#FFF', fontSize: scale(15), fontWeight: '700' },
  formLabel: { fontSize: scale(12), fontWeight: '700', letterSpacing: 1, marginBottom: scale(6), marginTop: scale(12) },
  modalInput: { height: scale(52), borderRadius: borderRadius.md, borderWidth: 1, paddingHorizontal: scale(16), fontSize: scale(16) },
  textAreaInput: { height: scale(90), paddingTop: scale(14), textAlignVertical: 'top' },
  colorRow: { flexDirection: 'row', gap: scale(12), marginTop: scale(4) },
  colorDot: { width: scale(36), height: scale(36), borderRadius: scale(18), borderWidth: 3, alignItems: 'center', justifyContent: 'center' },
  logoPickerBtn: { borderRadius: borderRadius.md, borderWidth: 2, borderStyle: 'dashed', overflow: 'hidden', height: scale(100), alignItems: 'center', justifyContent: 'center' },
  logoPreview: { width: '100%', height: '100%' },
  logoPlaceholder: { alignItems: 'center', gap: scale(4) },
  logoPlaceholderText: { fontSize: scale(13), fontWeight: '500' },
  // Countries
  regionHeader: { flexDirection: 'row', alignItems: 'center', gap: scale(10) },
  regionTitle: { flex: 1, fontSize: scale(16), fontWeight: '700' },
  regionBadge: { paddingHorizontal: scale(8), paddingVertical: scale(3), borderRadius: scale(10) },
  regionBadgeText: { fontSize: scale(12), fontWeight: '700' },
  countryList: { marginTop: scale(10), gap: 0 },
  countryRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: scale(10), borderBottomWidth: 1, gap: scale(10) },
  countryFlag: { fontSize: scale(24) },
  countryName: { fontSize: scale(14), fontWeight: '600' },
  countryMeta: { fontSize: scale(11), marginTop: 1 },
  // Verification
  verifyThumb: { flex: 1, height: scale(64), borderRadius: scale(8), borderWidth: 1, alignItems: 'center', justifyContent: 'center', overflow: 'hidden', gap: scale(2) },
  verifyThumbImg: { width: '100%', height: scale(46) },
  verifyThumbLabel: { fontSize: scale(9), fontWeight: '600', paddingHorizontal: scale(2) },
  // New banner styles
  totalBanner: { marginHorizontal: scale(16), marginVertical: scale(8), borderRadius: scale(18), borderWidth: 1.5, padding: scale(16) },
  bannerIcon: { width: scale(50), height: scale(50), borderRadius: scale(16), alignItems: 'center', justifyContent: 'center' },
  bannerTitle: { fontSize: scale(17), fontWeight: '800' },
  bannerSub: { fontSize: scale(13), fontWeight: '500', marginTop: scale(2) },
  refreshBtn: { width: scale(42), height: scale(42), borderRadius: scale(14), alignItems: 'center', justifyContent: 'center' },
});
