...
export default function SellerSettingsScreen() {
  ...
  const [selectedDeliveryMethods, setSelectedDeliveryMethods] = useState<string[]>(deliveryMethods || []);
  ...
  useEffect(() => { setSelectedDeliveryMethods(deliveryMethods || []); }, [deliveryMethods]);

  useEffect(() => {
    if (user?.id) {
      fetchDeliveryMethods(Number(user.numericId || user.sellerId || 0)).then(methods => {
        if (methods && methods.length > 0) setSelectedDeliveryMethods(methods);
      }).catch(() => {});
    }
  }, [user?.id, fetchDeliveryMethods]);
  ...
  const toggleDeliveryMethod = useCallback((methodId: string) => {
    selection();
    setSelectedDeliveryMethods(prev => prev.includes(methodId) ? prev.filter(id => id !== methodId) : [...prev, methodId]);
  }, []);

  const handleSaveDeliveryMethods = useCallback(async () => {
    setSavingMethods(true);
    await updateDeliveryMethods(selectedDeliveryMethods);
    setSavingMethods(false);
    notifySuccess();
  }, [selectedDeliveryMethods, updateDeliveryMethods]);
  ...
  return (
    <SafeAreaView edges={['top']} style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: scale(16), paddingBottom: insets.bottom + scale(40) }}>
        ...
        {/* Delivery Methods */}
        <View style={[styles.section, { backgroundColor: colors.surface, borderColor: colors.borderLight }, shadows.card]}>
          <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>{lb('Delivery Methods', 'Méthodes de livraison', 'طرق التوصيل')}</Text>
          <View style={{ gap: scale(8) }}>
            {DELIVERY_METHODS.map(method => {
              const isSelected = selectedDeliveryMethods.includes(method.id);
              const name = isFr ? method.name.fr : isAr ? method.name.ar : method.name.en;
              const desc = isFr ? method.description.fr : isAr ? method.description.ar : method.description.en;
              return (
                <Pressable
                  key={method.id}
                  onPress={() => toggleDeliveryMethod(method.id)}
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.shippingName, { color: colors.textPrimary }]}>{name}</Text>
                    <Text style={[styles.shippingPhone, { color: colors.textTertiary }]}>{desc}</Text>
                  </View>
                  <MaterialIcons name={method.icon as any} size={scale(18)} color={isSelected ? colors.primary : colors.textTertiary} />
                </Pressable>
              );
            })}
            <Pressable onPress={handleSaveDeliveryMethods} disabled={savingMethods} style={({ pressed }) => [styles.saveBtn, { backgroundColor: colors.primary, opacity: savingMethods ? 0.6 : pressed ? 0.85 : 1 }]}>
              {savingMethods ? <ActivityIndicator size="small" color="#FFF" /> : <Text style={styles.saveBtnText}>{lb('Save Delivery Methods', 'Enregistrer', 'حفظ طرق التوصيل')}</Text>}
            </Pressable>
          </View>
        </View>
