...
export default function CheckoutScreen() {
  ...
  const sellerPayments = seller?.paymentMethods || [];
  const availableMethods = useMemo(() =>
    sellerPayments.map(sp => {
      const method = paymentMethodsList.find(m => m.id === sp.methodId);
    ...
    [sellerPayments, paymentMethodsList]
  );
        ...
        {step === 'shipping' ? (
          <View>
            ...
            <View style={styles.cityGrid}>
              {["N'Djamena", 'Moundou', 'Abeche', 'Sarh', 'Kelo', 'Koumra', 'Pala', 'Bongor', 'Mongo', 'Doba'].map(city => (
        ...
        {step === 'payment' ? (
          <View>
            ...
            <Pressable onPress={() => setStep('shipping')} style={styles.backLink}>
              <MaterialIcons name="arrow-back" size={scale(16)} color={colors.textSecondary} />
