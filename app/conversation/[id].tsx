...
export default function ConversationScreen() {
  ...
  const _mockSeller = conv ? getSellerById(conv.sellerId) : undefined;
  // Load saved seller profile (uploaded logo)
  const [savedSellerProfile, setSavedSellerProfile] = React.useState<any>(null);
  useEffect(() => {
    if (conv?.sellerId) {
      AsyncStorage.getItem(`sokchad_seller_profile_${conv.sellerId}`).then(data => {
        if (data) { try { setSavedSellerProfile(JSON.parse(data)); } catch (e) {} }
      }).catch(() => {});
    }
  }, [conv?.sellerId]);
  // Fallback seller: if not in mock DB, build from product's sellerName or conversation data
  // Priority: savedSellerProfile.avatar (uploaded logo) > product.sellerAvatar > default
  const seller = conv
    ? (_mockSeller || {
        id: conv.sellerId || '',
        name: savedSellerProfile?.name || (product as any)?.sellerName || conv.sellerName || 'Seller',
        avatar: savedSellerProfile?.avatar || (product as any)?.sellerAvatar || '',
        storeBg: savedSellerProfile?.storeBg || '',
        sellerId: conv.sellerId || '',
        isVerified: savedSellerProfile?.isVerified || false,
        isBanned: false,
