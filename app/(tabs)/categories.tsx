...
export default function CategoriesScreen() {
  ...
  return (
    <SafeAreaView edges={['top']} style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingBottom: scale(14), borderBottomColor: colors.border }]}>
        <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>{lb('Categories', 'Catégories', 'الأقسام')}</Text>
        <Text style={[styles.headerSubtitle, { color: colors.textTertiary }]}>{displayCategories.length} {lb('categories', 'catégories', 'قسم')}</Text>
      </View>
      <FlatList
