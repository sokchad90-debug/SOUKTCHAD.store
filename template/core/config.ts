// @ts-nocheck
import { SokchadConfig } from './types';

class ConfigManager {
  private static instance: ConfigManager;
  private config: SokchadConfig | null = null;
  ...
  public initialize(config: SokchadConfig) {
    if (this.config) {
  ...
  public getConfig(): SokchadConfig {
    if (!this.config) {
  ...
  private createDefaultConfig(): SokchadConfig {
  ...
  public updateConfig(updates: Partial<SokchadConfig>) {
    const config = this.getConfig();
...
export const createConfig = (options: CreateConfigOptions = {}): SokchadConfig => {
  let authConfig;
