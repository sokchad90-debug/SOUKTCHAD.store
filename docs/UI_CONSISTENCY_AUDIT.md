...
## التكيّف الشامل — v8.9.38 (فرع adaptive-layout-v8.9.37)

### ما عالج (أسباب الاختلاف بين الأجهزة):
1. **صور مقصوصة من الخادم**: روابط unsplash كانت تطلب fit=crop (مربعات قسرية) → أُزيلت (33 رابط) وأُضيف resize يحافظ على النسبة (w=800&q=75&fm=jpg) — الصورة الأصلية كاملة عبر contain بلا تمديد.
2. **contentFit=cover كان يقص**: ProductImage صار contain + إطار ثابت أثناء التحميل/الفشل (لا قفز قائمة) + خلفية محايدة محسوبة عند اختلاف النسبة.
3. **فك ترميز صور أصلية ثقيلة** على الأجهزة الضعيفة كان سبب بطء/ANR ظاهري: التخزين المؤقت للخادم w=800 قلل الحجم ~80%.
4. **مصدر تصميم واحد**: ui/designSystem.ts (ألوان/خطوط/مسافات/زوايا/نسب) — الهوية من مكان واحد.

### مصفوفة الأجهزة (نفس APK vc126/v8.9.38):
| dp | الجهاز | اللغة | أعلى | نهاية | ملاحظة |
|---|---|---|---|---|---|
| 320 | small_old | FR | PASS | PASS | fallback صورة مفقودة سليم |
| 360 | hd_phone | AR | PASS | PASS | RTL مرآة، أسعار معزولة |
| 360+fs160 | hd_phone | AR | PASS | — | أسعار كاملة، لا تداخل |
| 393 | sokchad_x86 | FR | PASS | PASS (سابق) | المرجع |
| 411 | test_412_api30 | FR | PASS | PASS | 3-button nav |
| 432 | test_432_api30 (مخصص 400dpi) | FR | PASS* | — | *حوار ANR نظامي مستمر يغطي — الحالة خلفه سليمة وFATAL=0 |
| 601 | large_wide | FR | PASS | — | سقف 480dp متمركز + عمودان محفوظان |

### حدّات صادقة:
- تدوير الشاشة/تغيير حجم النافذة: غير مدعوم من النظام الحالي (portrait locked) — قيد موثق.
- API23/Android6: غير مدعوم رسمياً (RN 0.79/minSdk24) — لم يُختبر.
- جهاز حقيقي: بانتظار تثبيت المالك لهذه النسخة التجريبية.
- حوار "System UI isn't responding" على المحاكيات الباردة = قيد SwiftShader المضيف، التطبيق خلفه يعمل كاملاً (FATAL=0 في كل الجلسات).

## تصحيحات مراجعة v8.9.38 (بعد فحص المرجعين بالتكبير)

1) **الشارات على A54**: الفحص المكبر لـGOLDEN_FR_v2 وGOLDEN_AR_v2 يُظهر أن بطاقة A54 في المرجعين تحمل **القلب فقط** (أبيض داخل دائرة داكنة) — بلا دبوس وبلا شارة توثيق. مع ذلك طلب المالك «إعادة الشارات عند استحقاقها»: تواريخ البذور كانت منتهية (2025) فجُدّدت إلى 2027 — الآن الدبوس والتوثيق يظهران على A54 لأن بياناتها تستحق ذلك فعلاً، وبقية المنتجات غير المثبتة/غير الموثقة لا تحصل على شارات (البوابة isPinActive/isSellerVerificationActive تعمل).

2) **Studio à Louer**: الرابط سليم (HTTP 200 من الخادم). الفشل في لقطة FR كان **تحميلاً لم يكتمل** على إقلاع بارد بكاش فارغ وليس عطلاً ولا علاقة للغة. الإصلاح: إعادة محاولة تلقائية واحدة في ProductImage قبل إظهار البديل — يميّز فشلاً عابراً عن صورة مفقودة فعلاً.

3) **تصحيح توثيق RTL**: القائمة ☰ والدبوس في المرجع العربي على **يسار الشعار** (وليس يمينه كما وثّق سابقاً)، الجرس والسلة على يمينه. اعتمد GOLDEN_AR_v2 مرجعاً للمواضع.

4) **ANR**: سُجل كـ«متعطل بسبب بيئة الاختبار» وليس نجاحاً بصرياً — FATAL=0 لا ينفي ANR؛ السبب الفعلي: SystemUI على SwiftShader مع أجهزة AVD باردة + منافسة موارد المضيف. إعادة الاختبار تمت على إقلاعات نظيفة بلا لقطات مغطاة.
