...
## توحيد بطاقة↔تفاصيل المنتج — v8.9.56 (vc144)

### سبب التعارض (من مصدر البيانات):
- البطاقة تعرض product.rating/reviewsCount/soldCount من **بيانات الكتالوج** (mockData/DB).
- صفحة التفاصيل كانت تعرض getReviewsForProduct() = **التقييمات المكتوبة محلياً فقط** (فارغة في DB) → «Aucun avis» رغم 31 مراجعة في الكتالوج. ليست مشكلة لغة ولا جهاز.

### الإصلاح (من المصدر، لا نصوص ثابتة):
- صف التفاصيل: التقييم يستخدم كتالوج المنتج كمرجع (4.7 (31))، وصف المبيعات 214 vendus أُضيف، Top بشروط البطاقة (rating≥4.5 && sold≥100) في مساري السعر.
- المخزون صفر = «Rupture de stock» (حالة حقيقية) — الضمان يظهر عند وجوده فقط (warrantyDays).
- لوحة التقييمات: تحميل/خطأ/لا توجد مكتوبة — حالات منفصلة؛ مع بيانات كتالوج تعرض «Note catalogue 4.7 sur 31 acheteurs» بصدق.
- المفضلة من نفس toggleFavorite (تزامن تلقائي). الضغط على التقييم يفتح لوحة مراجعات المنتج نفسه.

### نتائج الاختبار (393dp FR، نفس APK vc144):
| الحالة | النتيجة |
|---|---|
| A54 كامل المعلومات (بطاقة↔تفاصيل متطابقة) | ناجح |
| «Aucun avis» الزائف | مُصلح — يعرض 4.7 (31) |
| 214 vendus في التفاصيل | ناجح (كان مفقوداً) |
| Top badge في التفاصيل | ناجح (كان مفقوداً) |
| المخزون/Max/Tوصيل | ناجح |
| فتح من مسارات متعددة | home ✓ (بحث/فئات/متجر: نفس المسار بالمعرف) |
| AR/fs200/بلا مراجعات/نفاد مخزون | لم يُختبر بعد — القواعد مطبقة في الكود |
