...
const MemoProductCard = React.memo(({ product, index }: { product: Product; index: number }) => (
  <ProductCard product={product} index={index} imageHeightRatio={RECENTLY_ADDED_IMG_RATIO} />
));
...
function HomeListHeader({
  ...
  return (
    <View>
      {/* No filter pills shown — badge on filter button indicates count */}

      {/* ===== City Picker Row ===== */}
      <View style={[styles.cityPickerRow, isAr && { flexDirection: 'row-reverse' }]}>
        <Pressable
          onPress={() => { selection(); setShowCityDropdown(true); }}
          style={({ pressed }) => [styles.cityPickerPill, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.88 : 1 }]}
        >
          <MaterialIcons name="location-on" size={14} color="#8B5CF6" />
          <Text style={[styles.cityPickerText, { color: selectedCity === 'all' ? colors.textSecondary : colors.textPrimary }]} numberOfLines={1}>
            {selectedCity === 'all'
              ? lb('All Cities', 'Toutes les villes', 'كل المدن')
              : selectedCity}
          </Text>
          <MaterialIcons name="expand-more" size={16} color={colors.textTertiary} />
        </Pressable>
      </View>
      ...
      {/* Flash Deals Banner — small, between search and categories */}
...
export default function HomeScreen() {
  ...
  const {
    ...
    filters, setFilters, resetFilters, activeFilterCount, isReady,
  ...
  const renderListHeader = useCallback(() => (
    <HomeListHeader
      ...
      products={stablePinnedProducts.length > 0 ? stablePinnedProducts : []}
      moreArrow={moreArrow}
  ...
  // Hide splash ONCE when isReady becomes true — after InteractionManager
  // completes pending interactions so the home screen is fully painted.
  // This is the ONLY place splash is hidden. No intermediate screens.
    ...
    <SafeAreaView edges={['top']} style={[styles.safeArea, { backgroundColor: colors.background }]}>
      <View style={{ flex: 1 }}>
          ...
          <FlatList
            ...
            contentContainerStyle={{ paddingTop: STICKY_HEADER_OFFSET, paddingBottom: tabBarHeight + scale(4) }}
...
const styles = StyleSheet.create({
  ...
  categoryGrid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: scale(16), paddingVertical: scale(4), gap: 0, marginTop: scale(2) },
  categoryGridItem: { alignItems: 'center', width: '25%', marginBottom: scale(4) },
  ...
  categoryCircleLabel: { fontSize: scale(10), marginTop: scale(4), textAlign: 'center', fontFamily: 'Cairo-Regular' },
  ...
  sectionHeader: { paddingHorizontal: scale(16), paddingTop: 0, paddingBottom: scale(2) },
  sectionHeaderRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', alignSelf: 'stretch', width: '100%', paddingHorizontal: scale(16), paddingTop: 0, paddingBottom: scale(2) },
  ...
  cityPickerRow: {
    ...
    paddingTop: scale(8), paddingBottom: scale(4),
  },
  cityPickerPill: {
    flexDirection: 'row', alignItems: 'center', paddingHorizontal: scale(12), paddingVertical: scale(6),
    borderRadius: scale(20), borderWidth: 1, gap: scale(6), maxWidth: SCREEN_WIDTH - scale(32),
  },
  cityPickerText: {
    fontSize: scale(12), fontWeight: '600', fontFamily: 'Cairo-SemiBold', flexShrink: 1,
  },
  // City dropdown modal
  cityDropdownOverlay: {
