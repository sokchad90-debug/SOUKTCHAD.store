...
-- Patch existing installations: add columns if they don't exist (idempotent)
-- MySQL 8+ supports IF NOT EXISTS via ALTER; for older, these are safe to re-run after checking
-- The PHP code also uses columnExists() fallback, so missing columns won't crash.
