...
import {
  ...
  products as mockProducts, conversations as mockConversations,
  mockOrders, sellers as mockSellers, paymentMethods, mockReviews,
  categories as mockCategories, Category,
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateStoreLogo = useCallback(async (uri: string) => {
    ...
    const finalUrl = uploadedUrl || uri;
    setUser(prev => { if (!prev) return null; return { ...prev, avatar: finalUrl }; });
    try {
      await apiUpdateProfile({ avatar_url: finalUrl });
      // Update AsyncStorage
  ...
  const updateStoreBanner = useCallback(async (uri: string) => {
    const uploadedUrl = await uploadImage(uri);
    const finalUrl = uploadedUrl || uri;
    setUser(prev => { if (!prev) return null; return { ...prev, coverImage: finalUrl }; });
    try {
      await apiUpdateProfile({ cover_url: finalUrl });
      // Update AsyncStorage
      const savedUser = await AsyncStorage.getItem('sokchad_user').catch(() => null);
      if (savedUser) {
        const parsed = JSON.parse(savedUser);
  ...
  const loadProductsFromDB = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data.length > 0) {
        // DB products take priority. Only keep local-only mock products
        // (non-UUID IDs) that don't overlap with DB product IDs.
        // This prevents image/title mismatches from stale mock data.
        setProducts(prev => {
          const dbIds = new Set(data.map(p => p.id));
          // Keep only mock products (short IDs like 'p1', 'p2') that aren't in DB
          const localOnly = prev.filter(p => !dbIds.has(p.id) && !p.id.match(/^[0-9a-f]{8}-[0-9a-f]{4}-/));
          // DB products first (correct images from API), then remaining mock products
          return [...data, ...localOnly];
        });
        setDbProductsLoaded(true);
      } else if (data.length === 0 && !error) {
        // DB is empty, keep mock data as demo
        setDbProductsLoaded(true);
      }
    } catch (e) {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          if (!cancelled && Array.isArray(convs) && convs.length > 0) {
  ...
  const fetchFollowStatus = useCallback(async (sellerId: string) => {
    try {
      const token = await AsyncStorage.getItem('sokchad_auth_token');
      if (!token) return { following: false, notifications_enabled: false };
      const res = await fetch(`https://souktchad.shop/api/follows.php?seller_id=${sellerId}`, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      const data = await res.json();
      if (data.success && data.data) {
        return { following: !!data.data.following, notifications_enabled: !!data.data.notifications_enabled };
      }
    } catch (e) { console.log('fetchFollowStatus error:', e); }
    return { following: false, notifications_enabled: false };
  }, []);
  ...
  const addProduct = useCallback(async (product: Omit<Product, 'id' | 'postedDate' | 'isPinned' | 'isFeatured' | 'views'>) => {
    ...
    if (user?.id) {
      const { data: dbProduct, error } = await dbCreateProduct(product, user.id);
      if (!error) {
        // Add product locally WITHOUT full reload — prevents FlatList scroll jump
        // The product is saved in DB, other users will see it on their next app open
        const newProd: Product = {
          ...product,
          id: dbProduct?.id || `p${Date.now()}`,
        ...
        console.log('DB create failed, falling back to local:', error);
      }
    }
    // Fallback: local-only product
    const newProduct: Product = { ...product, id: `p${Date.now()}`, postedDate: new Date().toISOString().split('T')[0], isPinned: false, isFeatured: false, views: 0, stock: (product as any).stock || 0 };
  ...
  const addPaymentMethod = useCallback((name: string, color: string, instructions: string, logoUrl?: string) => {
    ...
    setPaymentMethodsList(prev => [...prev, { id, name, logo: logoUrl || '', color, instructions }]);
  }, []);
  ...
  return (
    <AppContext.Provider value={{
      ...
      banUserWithDuration, addPaymentMethod, updatePaymentMethodLogo, removePaymentMethod, paymentMethodsList,
